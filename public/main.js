...
async function getPosts() {
	let res = await fetch('http://php.localhost:8000/posts');
...
async function addPost() {
	...
	const res = await fetch('http://php.localhost:8000/posts', {
		method: 'POST',
		body: formData
	});
...
async function removePost(id) {
	const res = await fetch(`http://php.localhost:8000/posts/${id}`, {
		method: "DELETE"
	});
